...
class ForcingFunction:
    ...
    def random_pulse(self, amplitude_range: Tuple, duration_range: Tuple = None):
        ...
        dur = (
            duration_range[0]
            + (duration_range[1] - duration_range[0]) * np.random.rand()
        )
    ...
    def to_spline_coeff(self):
        ...
        y_interp = torch.tensor(
            [self.__call__(x_interp[k]) for k in range(len(x_interp))]
        )
...
def construct_prc_pulses(
    amplitude, duration: float, period: float, samples: int = 10, num_periods: int = 2
):
    ...
    for k in range(samples):
        ...
        L1.add_pulse(
            pulse_indx[k],
            min(pulse_indx[k] + duration, num_periods * period),
            amplitude,
        )
        Pulses.append(L1)
    return Pulses
...
class PRCData(Dataset):
    def __init__(
        ...
        self.forcing_funcs = perturbations
        self.phases = [
            torch.tensor([ics[1], phase_shifts[k]]) for k in range(len(phase_shifts))
        ]
        self.t_data = []
        self.initial_cond = []
        self.coeffs = []
...
# get path within the package using resources


import deepPRC.prcdata


def loadComas(durations: List[int] = [1, 3, 4, 6, 9, 12, 18], datapath: Path = None):

    if datapath is None:
        datapath = importlib.resources.files('deepPRC.prcdata')

    shifts = []
    pulses = []

    for duration in durations:
        filename = datapath / ("Comas2006" + str(duration) + "Hr.csv")
...
def find_prc(
    model: nn.Module, pulses: List[ForcingFunction], ics: torch.Tensor
) -> np.ndarray:
    final_phases = []
    ...
    trajectory = (
        odeint(model, u0, ts, rtol=1e-9, atol=1e-9, method="rk4").detach().cpu()
    )
...
class PolarModel(nn.Module):
    ...
    def forward(self, t, state):
        ...
        pulsePhase = (
            self.sigma
            - self.A[0] * 0.5 * (R**3 + 1.0 / R) * torch.sin(psi + self.B[0])
            - self.A[1] * 0.5 * (1.0 + R**8) * torch.sin(2.0 * psi + self.B[1])
        )

        dR = (
            -self.gamma * R
            + self.K * torch.cos(self.beta) / 2.0 * R * (1 - R**2)
            + fvalue * pulseAmp
        )
        dPsi = (
            self.omega
            + self.K * torch.sin(self.beta) / 2.0 * (1 + R**2)
            + fvalue * pulsePhase
        )
...
# %% ../nbs/01_datatools.ipynb 17
class HopfOscillator(nn.Module):
    def __init__(self, mu: float = 1.0, tau: float = 24.0, epsilon: float = 0.5):
        super().__init__()
        self.omega = torch.tensor(2.0 * torch.pi / tau)
    ...
    def to_polar(self, trajectory):
        return torch.stack(
            (
                torch.sqrt(trajectory[:, 0] ** 2 + trajectory[:, 1] ** 2),
                torch.atan2(trajectory[:, 1], trajectory[:, 0]),
            ),
            dim=1,
        )

# %% ../nbs/01_datatools.ipynb 18
class KuramotoNetworkModel(nn.Module):
    def __init__(
        self, tau: float = 24.0, K: float = 0.30, beta: float = 0.02, gamma: float = 0.1
    ):
        super().__init__()
        self.omega = torch.tensor(2.0 * torch.pi / tau)
        self.K = torch.tensor(K)
...
class VDP(nn.Module):
    ...
    def forward(self, t, state):
        ...
        dxc = (
            ...
            / 12.0
            * (
                self.mu * (xc - 4.0 / 3.0 * xc**3.0)
                - x * ((24.0 / (0.99669 * self.tau)) ** 2.0 + self.kparam * Bhat)
            )
        )
...
class MorrisLecar(nn.Module):
    ...
    def forward(self, t, y):
        V, n = y
        dVdt = (1 / C) * (
            I - g_Ca * m_inf(V) * (V - V_Ca) - g_K * n * (V - V_K) - g_L * (V - V_L)
        )
