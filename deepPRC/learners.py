...
from .layers import *
from .datatools import SimPRCData, PRCData, loadComas, find_prc
...
class PRCLearner(pl.LightningModule):
    ...
    def forward(self, ic, ts, coeffs, ts_coeffs):
        ...
        yout = odeint(self.model, ic, ts, method="rk4", options=dict(step_size=1.0))
    ...
    def training_step(self, batch, batch_idx):
        ...
        for k in range(batch_t.shape[0]):
            pred_y[k, ...] = self.forward(
                batch_y0[k], batch_t[k], batch_coeff[k], batch_ts_coeffs[k]
            )
    ...
    def plot(self):
        ...
        for j, b in enumerate(self.data):
            ...
            pred_y = self.forward(
                batch_y0, batch_t, batch_coeff, batch_coeff_ts
            ).detach()
            ax.plot(batch_t, torch.sin(pred_y), ls="--", alpha=0.50, color="red")
        plt.show()

    def train_dataloader(self):
        trainloader = DataLoader(
            self.data, batch_size=self.batch_size, shuffle=True, num_workers=7
        )
        return trainloader

# %% ../nbs/02_learners.ipynb 9
def train_mouse(use_ann: bool = True, epochs: int = 30):
    ics = torch.tensor([0.70, 0.0])
    train_durations = [3, 6, 18]
    test_durations = [4, 9, 12]

    data = loadComas(durations=train_durations)
    print(f"Pretraining without neural networks for {epochs} epochs")
    model = SmallPerturbationModel(
            tau=23.8, use_nn_autonomous=True, use_nn_perturbation=True, use_tanh=False
        )
    model.freeze_nn() # freeze the neural network components
    trainer = pl.Trainer(
        max_epochs=epochs,
        log_every_n_steps=1,
        accelerator="cpu",
        #  callbacks=[FineTuneLearningRateFinder(milestones=list(range(0, epochs, 5)))],
    )
    model_learner = PRCLearner(model, data, batch_size=12, learning_rate=1e-2)
    trainer.fit(model_learner)
    
    plot_prc_mouse(train_durations, test_durations, model_learner.model, ics, name="mouse_nonn")
    if use_ann:
        model_learner.model.freeze_oscillator_params() # freeze the oscillator components
        model_learner.model.unfreeze_nn() # unfreeze the neural network components
        ...
        trainer.fit(model_learner)
    print(model_learner.model)
    # increase the batch size and train for more epochs
    ...
    plot_prc_mouse(train_durations, test_durations, model_learner.model, ics, name="mouse_full")

# %% ../nbs/02_learners.ipynb 11
def train_sim_model(model: torch.nn.Module, use_ann: bool = True, epochs: int = 30):
    # Define a training dataset
    train_experiments = [
        construct_prc_pulses(0.2, 1.0, 24.0, samples=12, num_periods=2),
        construct_prc_pulses(0.1, 1.0, 24.0, samples=12, num_periods=2),
    ...
    test_pulses = [
        construct_prc_pulses(0.7, 1.0, 24.0, samples=24),
    ...
    if use_ann:
        learned_model = SmallPerturbationModel(
            tau=24.0,
            use_nn_perturbation=True,
            inner_layers_forcing=10,
            use_nn_autonomous=False,
        )
    else:
        learned_model = SmallPerturbationModel(
    ...
    model_learner = PRCLearner(
        learned_model, sim_data, batch_size=12, learning_rate=1e-2
    )
    ...
    for idx, pulse_exp in enumerate(train_experiments):
        ...
        ax1.plot(
            np.linspace(0, 2 * torch.pi, len(prc1)),
            prc1,
            color=cmap(idx / len(total_experiments)),
        )
        ax1.scatter(
            np.linspace(0, 2 * torch.pi, len(prc_exp)),
            prc_exp,
            color=cmap(idx / len(total_experiments)),
        )
