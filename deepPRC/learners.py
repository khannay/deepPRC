...
# %% auto 0
__all__ = ['find_prc', 'PRCLearner', 'train_mouse', 'train_sim_model']
...
def find_prc(learner,
    ...
    learner.model.update_coeff(coeffs, ts_coeffs)
    u0 = ics  # learner.InitialCond(torch.tensor(ics)[1])
    ts = torch.linspace(fzero.start, fzero.stop, 3000)
    trajectory = odeint(learner.model, u0, ts, rtol=1e-9,
                        atol=1e-9, method='rk4').detach().cpu()
    zero_phase = trajectory[-1, 1]  # learner.MacroPhase(trajectory[-1, :])
    ...
    for f in pulses:
        ...
        trajectory = odeint(learner.model, u0, ts, rtol=1e-9,
                            atol=1e-9, method='rk4').detach().cpu()
        #final_phases.append(learner.MacroPhase(trajectory[-1, :])-zero_phase)
        final_phases.append(trajectory[-1, 1] - zero_phase)
...
def train_sim_model(
    ...
    for (idx, pulse_exp) in enumerate(train_experiments):
        prc1 = find_prc(model_learner, pulse_exp, ics)
    ...
    for (idx, test_pulse) in enumerate(test_pulses):
        ...
        prc1 = find_prc(model_learner, test_pulse, ics)
