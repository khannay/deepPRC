...
class MicoscopicModel(nn.Module):
    ...
    def forward(self, t, state):
        ...
        Q = (
            self.A[0] * torch.sin(phi)
            + self.A[1] * torch.sin(2.0 * phi)
            + self.B[0] * torch.cos(phi)
            + self.B[1] * torch.cos(2 * phi)
        )
        ...
        dphi = (
            ...
            / self.N
            * torch.sum(
                self.K * torch.sin(phi[None, :] - phi[:, None] + self.beta), axis=0
            )
            + Q * lval
            - correctionTerm
        )
    ...
    def __repr__(self) -> str:
        ...
        for name, param in self.named_parameters():
            if param.requires_grad:
                my_rep += (
                    "Parameter " + str(name) + ", has value: " + str(param.data) + "\n"
                )
        return my_rep
...
class SmallPerturbationModel(nn.Module):
    ...
    def __init__(
        ...
        # Fitable Parameters, init from zero values to avoid nan
        ...
        self.neuralNetwork = nn.Sequential(
            nn.Linear(2, inner_layers_forcing),
            nn.GELU(),
            nn.Linear(inner_layers_forcing, inner_layers_forcing),
            nn.GELU(),
            nn.Linear(inner_layers_forcing, 2),
        )
        self.neuralNetwork.apply(self.init_weights_zero)

        self.autonomousTerm = nn.Sequential(
            nn.Linear(2, inner_layers_autonmous),
            nn.GELU(),
            nn.Linear(inner_layers_autonmous, inner_layers_autonmous),
            nn.GELU(),
            nn.Linear(inner_layers_autonmous, 2),
        )
        self.autonomousTerm.apply(self.init_weights_zero)

        if not use_nn_perturbation:
            for param in self.neuralNetwork.parameters():
                param.requires_grad = False
        if not use_nn_autonomous:
            for param in self.autonomousTerm.parameters():
                param.requires_grad = False
    ...
    def freeze_oscillator_params(self):
        """
        Freeze the oscillator parameters
        """
    ...
    def unfreeze_oscillator_params(self):
        """
        Unfreeze the oscillator parameters
        """
    ...
    def forward(
        ...
        LightAmp = self.A[0] * 0.5 * (1.0 - R**4.0) * torch.cos(
            Psi + self.B[0]
        ...
        dR = (
            -1.0 * self.gamma * R
            + self.K * torch.cos(self.beta) / 2.0 * R * (1.0 - R**4)
            + LightAmp * PulseValue
        )
        dPsi = (
            2 * torch.pi / self.tau
            + self.K / 2.0 * torch.sin(self.beta) * (1 + R**4)
            + LightPhase * PulseValue
        )
        ...
        if self.use_nn_perturbation and perturbation_nn:
            ...
            if self.use_tanh:
                ...
                dfunc += neural_term_perturbation * PulseValue
        if self.use_nn_autonomous and autonomous_nn:
            ...
            if self.use_tanh:
                ...
                dfunc += autonomous_term_perturbation
        return dfunc
...
class PureANN(nn.Module):
    ...
    def forward(self, t, state):
        ...
        autoTerm = torch.tensor([0.0, 2 * torch.pi / self.tau]) + self.autonomousANN(
            torch.sin(state)
        )
